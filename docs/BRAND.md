...
> **Sistema vigente (2026-06-24): entidad pública moderna alineada con gov.co — ver [ADR-021](./adr/021-sistema-diseno-gov-co.md).** Reemplaza el sistema *Civic Editorial* (papel & tinta, IBM Plex Serif, granate/ámbar). Identidad actual: tipografía **Nunito Sans** (oficial gov.co) + IBM Plex Mono para datos, superficie blanca, **azul institucional `#004884`**, semánticos gov.co para el semáforo, barra GovHead con atribución **textual** del Estado (sin logo gov.co ni escudo). Las secciones siguientes ya reflejan este sistema; donde aún se lea "papel/crema/serif/granate", prevalece lo dicho aquí y en ADR-021.
...
DatosVivos es un **agente civil de datos del Estado colombiano** sobre el catálogo integrado de datos abiertos (datos.gov.co + portales territoriales + IGAC). Presenta el panorama nacional a los tomadores de decisiones y permite al ciudadano preguntar en su idioma — el del barrio, el del trabajo, el del periódico — ejecutando consultas reales sobre los datos. Cada cifra que aparece está calculada a partir de filas verdaderas; cada respuesta cita el dataset que la sustenta con un enlace clicable. El servicio corre sobre infraestructura del Estado.
...
- **No es un dashboard promocional.** La home ES un panorama ejecutivo nacional (ADR-023): cifras vivas del catálogo, cada una con fuente y semántica auditables. Lo que nunca será es un panel de cifras decorativas sin fuente reproducible.
...
1. **Soberanía** — el servicio corre sobre infraestructura del Estado y la telemetría ciudadana es anónima. Los datos consultados son públicos.
...
| `--hairline` | Bordes, separadores |
| `--accent` | Azul institucional gov.co (texto, enlaces, header, énfasis) |
| `--accent-2` | Azul interactivo gov.co (rellenos de CTA, chips activos) |
| `--focus-ring` | Anillo de foco accesible (siempre visible, nunca outline:none) |
| `--danger` | Alertas, errores (rojo texto-seguro; siempre con icono y texto) |
| `--ok` / `--warn` / `--bad` | Semáforo de frescura del catálogo (verde / dorado / rojo gov.co) |

### 3.2 Modo claro (default) — institucional gov.co

| Token | Hex | Comentario |
|---|---|---|
| `--bg` | `#FFFFFF` | Blanco institucional (registro gov.co). |
| `--bg-elev` | `#F4F6FA` | Gris frío suave para tarjetas/paneles. |
| `--ink` | `#16161D` | Casi-negro neutro. Contraste con `--bg`: ~17:1 (AAA). |
| `--ink-2` | `#3D4351` | Texto secundario. ~9:1 (AAA). |
| `--ink-muted` | `#636B7A` | Metadata/placeholders. ~5:1 (AA). |
| `--hairline` | `#D6DBE3` | Borde gris frío. Rol: separar, no informar. |
| `--accent` | `#004884` | **Azul institucional gov.co.** Texto/enlaces/header. 9.3:1 (AAA). |
| `--accent-2` | `#3366CC` | **Azul interactivo gov.co.** Rellenos de CTA con texto blanco. 5.4:1 (AA). |
| `--focus-ring` | `#3366CC` | 2px con offset 2px. Visible en `:focus-visible`. |
| `--danger` | `#C32D4B` | Rojo texto-seguro (5.6:1 con texto blanco). El `#F42F63` brillante solo para puntos/no-texto. |
| `--ok` / `--warn` / `--bad` | `#068460` / `#FFAB00` / `#C32D4B` | Semáforo. `--warn` se usa con tinta oscura (9.5:1). |

### 3.3 Modo oscuro — derivado institucional

gov.co no define un modo oscuro oficial; se deriva conservando los hues y subiendo luminosidad.

| Token | Hex | Comentario |
|---|---|---|
| `--bg` | `#0E1422` | Navy casi-negro institucional. |
| `--bg-elev` | `#1A2433` | Capa de tarjeta. |
| `--ink` | `#E8ECF2` | ~15:1 (AAA). |
| `--ink-2` | `#B4BECE` | ~9:1 (AAA). |
| `--ink-muted` | `#7E8BA0` | ~4.7:1 (AA). |
| `--hairline` | `#2A3647` | Borde frío oscuro. |
| `--accent` | `#5B91FF` | Azul luminoso. ~6:1 (AA). |
| `--accent-2` | `#7FB0FF` | Azul claro para rellenos. |
| `--focus-ring` | `#7FB0FF` | Visible sobre fondos oscuros. |
| `--danger` | `#FF6B85` | ~5:1 (AA). |
| `--ok` / `--warn` / `--bad` | `#2BB17E` / `#FFC24A` / `#FF6B85` | Semáforo modo oscuro. |
...
| `--accent` | `#003A8C` (azul institucional oscuro, 10:1) |
| `--accent-2` | `#003A8C` |
| `--focus-ring` | `#003A8C` |
| `--danger` | `#9B0000` (8.8:1) |
...
Dos familias: **Nunito Sans** (tipografía oficial gov.co) para todo el texto, e **IBM Plex Mono** para datos. Se elimina IBM Plex Serif. Ambas libres, latín completo, sin asociación con AI tooling.

| Familia | Uso | Pesos |
|---|---|---|
| **Nunito Sans** | Display, h1-h6, body, UI controls, narrativa, wordmark | 400, 600, 700, 800 (+ italic 400) |
| **IBM Plex Mono** | Data, IDs de dataset, SoQL citado, kickers (`TEMA`, `TIPO`), números en tablas | 400, 500 |

Ambas se sirven **self-hosted** desde `web/public/fonts/` en formato `woff2`. Subset latin. Prohibido cargar fuentes desde Google Fonts CDN u otros CDN externos (requisito de privacidad ciudadana — el woff2 se descarga una vez y se hornea en la imagen).
...
|---|---|---|---|---|
| Display XL | Sans | `clamp(2.5rem, 5vw, 4rem)` | 1.05 | 800 |
| h1 | Sans | `clamp(2rem, 3.5vw, 3rem)` | 1.1 | 800 |
| h2 | Sans | `clamp(1.625rem, 2.8vw, 2.25rem)` | 1.15 | 800 |
| h3 | Sans | `clamp(1.3125rem, 2vw, 1.5rem)` | 1.25 | 700 |
...
Lenguaje moderno gov.co: esquinas suaves en superficies interactivas.

- `--radius-0`: `0` — separadores, regletas, detalles.
- `--radius-1`: `4px` — botones, inputs, badges.
- `--radius-2`: `8px` — tarjetas, paneles, tarjetas de dataset.
- `--radius-3`: `12px` — chips (pill suave).
- Sigue prohibido `rounded-full` en elementos no-circulares y los avatares "burbuja".
...
- Permitida una **elevación sutil** en tarjetas: `box-shadow: 0 1px 2px var(--bg-overlay)` (registro gov.co). Sin "soft shadows" Apple exageradas ni elevation Material de varias capas.
...
- Composición: Nunito Sans ExtraBold (800), kerning compacto (`letter-spacing: -0.02em`), color `var(--ink)`.
- La pleca se renderiza en `var(--accent)` (azul institucional) como acento.
- No hay logo gráfico separado. El wordmark **es** el logo. Es identidad de producto, distinta del lockup gov.co (que NO usamos).
...
### 9.1 Home `/` — Panorama nacional (ADR-023)

La home es el nivel 1 de la arquitectura de información (panorama → `/tablero` detalle →
`/buscar` dato puntual). Cero cajas de búsqueda: la búsqueda es un CTA. Los chips y
"Lo más consultado" viven en `/buscar` (§9.2).

```
┌─────────────────────────────────────────────────────────────────┐
│  Datos|Vivos                              Claro · Oscuro · Alto │
│  ── datos.gov.co          Tablero · Buscar · Acerca · A11y      │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   DATOS DEL ESTADO, EN TUS PALABRAS            ← kicker marca   │
│   El panorama de los datos abiertos                             │
│   de Colombia                                  ← h1             │
│   Cuántos hay, quién los publica, qué tan                       │
│   frescos están y cómo acceder a ellos.                         │
│                                                                 │
│   ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌───────────┐      │
│   │  21.996   │ │   1.4xx   │ │    xx %   │ │    xx %   │      │
│   │ datasets  │ │ entidades │ │  frescos  │ │consultable│      │  ← count-up
│   └───────────┘ └───────────┘ └───────────┘ └───────────┘      │
│   cifras en vivo · actualizado hace N min                       │
│                                                                 │
│   POR SECTOR                    │  POR DEPARTAMENTO             │
│   Salud        ████████ 4.1k   │   [ mapa coroplético ]        │
│   Educación    ██████ 3.2k     │   Top-5: Bogotá, Antioquia…   │
│   ...(top 10)                   │   + N de alcance nacional     │
│                                                                 │
│   FRESCURA  ██████░░░░▒▒ verde/amarillo/rojo/s.f.               │
│   ACCESO    ████████░░▒  directo/herramienta/metadatos          │
│                                                                 │
│   ┌──────────────────────────┐ ┌──────────────────────────┐    │
│   │ Explora el detalle       │ │ Pregunta en tus palabras │    │
│   │ por sector y entidad  →  │ │ y verifica la fuente  →  │    │  ← CTAs
│   │ /tablero                 │ │ /buscar                  │    │
│   └──────────────────────────┘ └──────────────────────────┘    │
│                                                                 │
...
Reglas: un solo fetch (`/api/v1/stats/panorama`, caché server 5 min); degradación sin crash
si la API no responde; animación de revelado según ADR-023 (una vez, ≤800ms,
reduced-motion la anula); sin filtros interactivos (el corte por entidad es de `/tablero`).
...
6. Sin **animaciones de aparición** (fade-up, slide-in, spring) en el flujo de búsqueda/resultados — la respuesta a una consulta no se "presenta", se entrega. **Excepción única (ADR-023):** revelado de datos en la home panorama (count-up, crecimiento de barras, fade del mapa): una sola vez al entrar al viewport, ≤800ms, valor final siempre en el HTML, anulada por `prefers-reduced-motion`. Spring/bounce/parallax siguen prohibidos en todas partes.
